    ...
    <Layout>
      <div className="space-y-8">
        ...
        <div className="border-b border-zinc-800/80 pb-4">
          <h1 className="text-xl font-bold text-white tracking-widest uppercase">BURN LAB</h1>
                ...
                return (
                  <button
                    ...
                    className={`relative text-left border p-2 space-y-2 transition-colors ${
                      isSelected
                        ? "border-[#CCFF00] bg-zinc-900 shadow-[inset_0_0_10px_rgba(204,255,0,0.08)]"
                  ...
                  >
                    ...
                    <input
                      ...
                      checked={isSelected}
                      readOnly
                      className="absolute top-2 right-2 accent-[#CCFF00] pointer-events-none"
                    />
                  </button>
                );
              })}
        ...
        <div className="border border-zinc-800 bg-[#0f1115] p-6 space-y-4">
          <div className="flex justify-between items-center flex-wrap gap-2">
            <h3 className="text-sm font-bold text-white tracking-widest">REWARD PREVIEW</h3>
            <span className="text-[10px] text-zinc-500">
              SELECTED: <span className="text-[#CCFF00] font-bold">{selectedCount} NFT{selectedCount === 1 ? "" : "s"}</span>
            </span>
          </div>
          ...
          {burnRewardsLoading ? (
            ...
            <div className="space-y-2">
              {activeRewards.map((r) => (
                <div key={r.token} className="flex justify-between text-xs p-2 bg-zinc-950 border border-zinc-900">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              {needsApproval ? (
                <button
                  ...
                  className="flex-1 py-3 bg-[#CCFF00] text-black font-bold text-xs tracking-widest disabled:opacity-30 disabled:cursor-not-allowed hover:bg-[#b8e600] transition-colors"
                >
                  APPROVE NFTS
                </button>
              ) : !showConfirm ? (
                ...
                <>
                  <button
                    ...
                    disabled={isBusy}
                    className="flex-1 py-3 bg-zinc-900 border border-zinc-800 text-zinc-400 font-bold text-xs tracking-widest hover:text-white disabled:opacity-30"
                  >
                    CANCEL
                  </button>
                  <button
