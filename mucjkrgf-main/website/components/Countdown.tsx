...
const Unit: React.FC<{ value: number; label: string }> = ({ value, label }) => (
  <div className="flex-1 min-w-[64px] border border-zinc-800 bg-zinc-950 py-3 text-center">
    <div className="text-2xl md:text-3xl font-bold text-[#CCFF00] tabular-nums">{pad2(value)}</div>
...
export const CountdownBadge: React.FC<{ target: string | number | undefined | null }> = ({ target }) => {
  ...
  if (isComplete) {
    return (
      <span className="ml-auto px-1.5 py-0.5 text-[9px] font-bold tracking-wider bg-[#CCFF00] text-black">
        LIVE
      </span>
    );
  }
  ...
  return (
    <span className="ml-auto px-1.5 py-0.5 text-[9px] font-bold tracking-wider border border-zinc-700 text-zinc-400 tabular-nums">
